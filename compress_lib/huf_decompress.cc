...
size_t HUF_decompress_float_internal_bf16 (void* dst, size_t dstSize, const void* cSrc, size_t cSrcSize) {
    if (dstSize == 0) return ERROR(dstSize_tooSmall);
    ...
    // 计算输入buffer上的的一些区域的起始指针
    const uint8_t* ip_e    = (const BYTE*)cSrc + 16;                 // 起始指针: huffman表, 跳过16byte元数据
    const uint8_t* ip_full = (const BYTE*)cSrc + e_len;              // 起始指针: 尾数 (全部为全精度)

    // 解析huffman表
    HUF_CREATE_STATIC_DTABLEX1(DTable, HUF_TABLELOG_MAX);
    U32 workSpace[HUF_DECOMPRESS_WORKSPACE_SIZE_U32];
    size_t hSize = HUF_readDTableX1_wksp(DTable, ip_e, (ip_full-ip_e), workSpace, sizeof(workSpace));
    if (HUF_isError(hSize)) return hSize;
...
static size_t HUF_decompress_float_bf16 (void* dst, size_t dstSize, const void* cSrc, size_t cSrcSize) {
    uint8_t* istart = (uint8_t*)cSrc;
    uint8_t* ip     = istart;
    uint8_t* iend   = istart + cSrcSize;
    ...
    while (ip < iend) {
        uint32_t srcSize     = ((((uint32_t*)ip)[0]) & 0X00FFFFFF);
        uint32_t segmentSize = ((((uint32_t*)ip)[1]) & 0X00FFFFFF);
        CHECK_V_F(decSize, HUF_decompress_float_internal_bf16(op, segmentSize, ip, srcSize));
        ip += srcSize;
        op += decSize;
    }
    return op - ostart;
...
size_t HUF_decompress_float (void* dst, size_t dstSize, const void* cSrc, size_t cSrcSize, DataType *p_dataType) {
    if (cSrcSize <= 16) {
        return 0;   // srcbuffer不够大, 连元数据都放不下，报错
    }
    DataType dt = (DataType)(((const uint32_t*)cSrc)[0] >> 24);
    ...
    switch (dt) {
        case DT_BF16: return HUF_decompress_float_bf16 (dst, dstSize, cSrc, cSrcSize);
        default:      return 0;
    }
}
